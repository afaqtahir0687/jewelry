...
export default function NavItem({
    ...
    const classes = `relative h-full flex items-center px-1 text-sm font-semibold transition-colors duration-200 group cursor-pointer ${isActive ? 'text-[#d4af37]' : 'text-[#4a0e0e] hover:text-[#d4af37]'}`;
                        ...
                        <ul className="flex flex-col">
                            {item.groups!.map((group, gIdx) => (
                                <li key={gIdx} className="relative group/sub">
                                    <button
                                        className="w-full flex items-center justify-between px-4 py-2 text-xs font-semibold uppercase tracking-wider text-[#4a0e0e] hover:bg-[#fff8f0] hover:text-[#d4af37] transition-all duration-300"
                                        aria-haspopup="true"
                                        aria-expanded="false"
                                    >
                                        {group.heading}
                                        ...
                                        <ul className="flex flex-col">
                                            {group.links.map((link, lIdx) => (
                                                <li key={lIdx}>
                                                    <Link
                                                        href={link.href}
                                                        className="block px-4 py-2 text-xs font-semibold uppercase tracking-wider text-[#4a0e0e] hover:bg-[#fff8f0] hover:text-[#d4af37] hover:pl-5 transition-all duration-300"
                                                        onClick={closeMenu}
                                                    >
                                                        {link.label}
                                                    </Link>
