...
export default function SellerProductEdit({ product, categories }: EditProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        // Since we are uploading files, we use POST with Laravel method spoofing PATCH (_method: PATCH)
        post(`/seller/products/${product.id}`);
    };

    return (
        <SellerLayout title={`Edit Design: ${product.title}`}>
            <Head title={`Edit Design — Seller`} />
            ...
            <div className="max-w-2xl space-y-6">
                ...
                <div className="rounded-xl border border-white/10 bg-white/5 p-6">
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="images">Upload New Photos (replace existing)</Label>
                            <input
                                id="images"
                                type="file"
                                multiple
                                onChange={(e) => setData('images', Array.from(e.target.files ?? []))}
                                className="block w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:bg-white/15 file:text-white file:cursor-pointer"
                            />
                        </div>

                        <Button type="submit" className="w-full mt-4" disabled={processing}>
                            {processing ? 'Saving...' : 'Save Product Design'}
                        </Button>
                    </form>
                </div>
            </div>
